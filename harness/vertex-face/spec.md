...
### 기능: 삼각형 에지 시각화 (팬 삼각분할 와이어프레임) — 리비전
- **의도**: 면이 어떻게 삼각형들로 쪼개지는지(팬 삼각분할)를 눈으로 봐야 학습이 된다. 채워진 면만 보면 정점 0에서 뻗는 내부 대각선이 안 보인다.
- **방식**: `FaceMesh.getTriangleEdges()`가 현재 index로부터 각 삼각형의 3변을 **중복 제거한 무방향 정점 인덱스 쌍**(`[a, b][]`)으로 반환한다. 페이지는 이 에지로 `LineSegments`를 만들어 면 위에 얹어, 외곽선(LineLoop)뿐 아니라 **내부 대각선까지** 그린다.
- **주의**: 평면 다각형은 모든 삼각형이 동일 평면이라 `EdgesGeometry`(코플래너 병합)로는 내부 대각선이 사라진다 → 삼각형 변을 직접 뽑아 그린다. 정점 수가 바뀌면 에지도 재계산한다.
- **인수기준**: N≥3일 때 `getTriangleEdges()`가 팬 삼각분할의 모든 삼각형 변을 담고(무방향·중복 없음), 에지 개수 = `2N-3`이다. N=4면 내부 대각선 `(0,2)`를 포함한다. N<3이면 빈 배열. **[사람 확인 필요]** 화면에서 각 삼각형의 경계선(내부 대각선 포함)이 보인다.
