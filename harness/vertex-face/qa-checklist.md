---
input_hash: a28d8a5eabcd489cd2782eac79f9ecad4ba5e98527d2db9f3f9a3f6d8d25f8a5
...
- **정점을 임의 순서로 찍어도 선(에지)이 서로 교차하지 않음(클릭순 LineLoop 제거, 경계는 삼각형 에지가 겸함)**
...
| 삼각형 에지 LineSegments 화면 표시(내부 변) | (테스트 불가 — `VertexFace.tsx` 배선 확인됨) | — 육안 확인 | 요검토 |
| **정점을 임의 순서로 찍어도 선(에지)이 서로 교차하지 않음(LineLoop 제거)** | (테스트 불가 — `VertexFace.tsx`에서 클릭순 `LineLoop` 삭제, 경계는 삼각형 에지가 겸함) | — 육안 확인 | 요검토 |
...
- **클릭순 외곽선(LineLoop) 제거(이번 갱신)**: `VertexFace.tsx`에서 클릭 순서로 정점을 잇던 닫힌 `LineLoop`를 삭제했다. 들로네는 클릭 순서와 경계 순서가 무관해 클릭순 고리가 자기교차(선이 서로 가로지름)하는 문제가 있었고, 바깥 경계는 삼각형 에지(볼록 껍질 변)가 이미 겸하므로 별도 외곽선이 불필요하다. **순수 로직·테스트에는 변동 없다**(렌더링/상호작용 층 수정, 테스트 37개 그대로 통과). 관련 인수기준(선이 서로 교차하지 않음 / 경계가 삼각형 에지로 드러남)은 여전히 `[사람 확인 필요]` 육안 항목 — 위 매트릭스에 '정점을 임의 순서로 찍어도 선(에지)이 서로 교차하지 않음' 항목으로 반영했다.
